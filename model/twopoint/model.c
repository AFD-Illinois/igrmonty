...
void init_data(int argc, char *argv[], Params *params)
{
  ...
  if (params->loaded) {
    lnumin = params->lnumin;
    lnumax = params->lnumax;
    alpha_spec = params->alpha_spec;
  }
...
void report_spectrum(int N_superph_made, Params *params)
{
  ...
  h5io_add_data_dbl_2d(fid, "/output/N", N_EBINS, N_THBINS, N_buf);
