...
// This file contains bremsstrahlung routines that are used only if the van Hoof 2015 + Nozawa 2009 formulae are used.
...
This code uses Gaung factor data from:
...
//  gsl_interp_accel *bremss_yacc;
  ...
  FILE *myfile;
  myfile = fopen("src/gff_ei_bremss.dat","r");
  if ( myfile == NULL ){
    myfile = fopen("gff_ei_bremss.dat", "r");
    if ( myfile == NULL ){
      perror("Tried to load Gaunt factor data from gff_ei_bremss.dat and src/gff_ei_bremss.dat but");
      exit(-1);
    }
  }
...
The following code is a C version of the one released in:
